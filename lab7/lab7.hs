...
isBloodOrange (Apple v value) = False
isBloodOrange (Orange v seg) = elem v tr

-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments xs = sum([ s | x@(Orange v s)<-xs,isBloodOrange x])

-- 3.
worms :: [Fruit] -> Int
worms xs = sum([1 | x@(Apple v val)<-xs, val == True])
...
p4 = ((p :->: q) :&: (p :<->: q)) --((P → Q) & (P ↔ Q))
p5 = ((p :->: q) :&: (p :&: (Not q))) --((P → Q) & (P & (¬Q)))
p6 = ((p :<->: q) :&: ((p :&: (Not q)) :|: ((Not p) :&: q))) --((P ↔ Q) & ((P & (¬Q)) ∨ ((¬P ) & Q)))

-- 7.
equivalent :: Prop -> Prop -> Bool
equivalent p1 p2 = and( [ (eval e p1) == (eval e p2) | e <- (envs (nub (names p1 ++ names p2))) ] )
